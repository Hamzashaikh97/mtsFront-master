...
import {ConcentrationMarginData} from './types';
import {CONCENTRATIONMARGINDATA} from './memberData';
import {DecimalPipe} from '@angular/common';
import {debounceTime, delay, switchMap, tap} from 'rxjs/operators';
import {SortColumn, SortDirection} from './sortable.directive';

interface SearchResult {
  ConcentrationMarginsData: ConcentrationMarginData[];
  total: number;
}
...
function sort(ConcentrationMarginsData: ConcentrationMarginData[], column: SortColumn, direction: string): ConcentrationMarginData[] {
  if (direction === '' || column === '') {
    return ConcentrationMarginsData;
  } else {
    return [...ConcentrationMarginsData].sort((a, b) => {
...
function matches(MarginData: ConcentrationMarginData, term: string, pipe: PipeTransform) {
...
export class MemberDataService {
  ...
  private ConcentrationMarginsData$ = new BehaviorSubject<ConcentrationMarginData[]>([]);
  ...
  constructor(private pipe: DecimalPipe) {
    this._search$.pipe(
      ...
      this.ConcentrationMarginsData$.next(result.ConcentrationMarginsData);
  ...
  get ConcentrationMarginData$() { return this.ConcentrationMarginsData$.asObservable(); }
...
//   set pageSize(pageSize: number) { this._set({pageSize}); }
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 2. filter
    ConcentrationMarginsData = ConcentrationMarginsData.filter(MarginData => matches(MarginData, searchTerm, this.pipe));
